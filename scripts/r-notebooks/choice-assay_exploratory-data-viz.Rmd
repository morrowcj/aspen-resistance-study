...
preliminary connections, until our chemistry analyses are completed. 
...
chem.data <- chem.data[-which(chem.data$Chem.ID %in% dup.IDs & chem.data$redo.flag != 1 ),]
## replace the "700 redo" SerialNo with the correct "700"
chem.data$SerialNo <- factor(gsub(chem.data$SerialNo,pattern = "700.*",replacement = "700"))
...
with(chem.data,{
  plot(c(Scin.DW + Scort.DW + Tdin.DW +Tcin.DW) ~ ordered.SN, ylab = "Total PG %")
})

# print a subset of the summary table
pg.summ.table[sample(1:nrow(pg.summ.table),size = 4),]

# summary(pg.summ.table)

```


```{r merge in chem data}
...
```{r}
with(data.reduced,{
  plot(Vol.2018 ~ group, main = "Current Volume by group")
